...
export function extractIngredientSection(rawText: string): { text: string; sectionFound: boolean } {
  ...
  for (const marker of ingredientMarkers) {
    ...
    if (match && (start < 0 || match.index < start)) start = match.index + match[0].length;
  }
  ...
  return { text: (end ? candidate.slice(0, end.index) : candidate).trim(), sectionFound: true };
}

export function describeOcrQuality(confidence: number, sectionFound: boolean): string {
  if (confidence < 55) return 'Low-confidence scan. Check every ingredient or try a sharper, closer photo.';
  if (!sectionFound) return 'Text extracted, but no ingredient heading was detected. Remove unrelated label text before analyzing.';
  if (confidence < 75) return 'Ingredient section detected. Review spelling and punctuation before analyzing.';
...
export async function prepareOcrImage(file: File, rotation: number, cropCenter: boolean): Promise<Blob> {
  ...
  bitmap.close();
  return await new Promise<Blob>((resolve) => canvas.toBlob((blob) => resolve(blob ?? file), 'image/png', 0.94));
}
